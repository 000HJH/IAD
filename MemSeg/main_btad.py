...
from data import create_dataset_btad, create_dataloader
...
def run(cfg):
    ...
    exp_name= cfg.EXP_NAME + f"-{cfg.DATASET.target}"
    savedir = os.path.join(cfg.RESULT.savedir, exp_name)
    os.makedirs(savedir, exist_ok=True)
    ...
    file_path = os.path.join(cfg.RESULT.savedir, '_result.csv')
...
if __name__=='__main__':
    ...
    for obj in obj_list:
        cfg.DATASET.target = obj
        cfg.DATASET = OmegaConf.merge(cfg.DATASET, target_cfg[cfg.DATASET.target])
        
        run(cfg)

    running_time = time.time() - start
    print(f'({cfg.DATASET.target}) 소요시간: {running_time//3600}시간 {(running_time%3600) // 60}분')
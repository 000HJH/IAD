...
class  MemSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img, fg_mask = self.generate_anomaly(file_path=file_path, img=img, texture_img_list=self.texture_source_file_list)
    ...
    def generate_anomaly(self, file_path, img: np.ndarray, texture_img_list: list = None) -> List[np.ndarray]:
        '''
        ...
        ## target foreground mask
        if self.use_mask:
            target_foreground_mask = self.generate_target_foreground_mask(file_path=file_path, img=img)
        else:
    ...
    def generate_target_foreground_mask(self, file_path, img: np.ndarray) -> np.ndarray:
        """
        ...
        #fg 파일
        classname = file_path.split('/')[-4]
        fgmask_path = file_path.split(classname)[0] + 'fg_mask/' + classname + '/' + os.path.split(file_path)[-1] 

        fgmask = cv2.imread(fgmask_path, cv2.IMREAD_GRAYSCALE)  # 2D grayscale 로드
        fgmask_resized = cv2.resize(fgmask, (img.shape[0], img.shape[1]), interpolation=cv2.INTER_NEAREST)
        fgmask_norm = fgmask_resized/255
        target_foreground_mask = (fgmask_norm > 0.5).astype(np.int32)  # 또는 np.int
...
class MemSegDataset_visa(Dataset):
    ...
    def generate_anomaly(self, file_path, img: np.ndarray, texture_img_list: list = None) -> List[np.ndarray]:
        ...
        if self.use_mask:
            ...
            target_foreground_mask = np.ones(self.resize) #288,288
        ## perlin noise mask
        perlin_noise_mask = self.generate_perlin_noise_mask(img_size=img_size)

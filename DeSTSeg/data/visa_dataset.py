...
class VisADataset(Dataset):
    def __init__(
        ...
        if is_train:
            self.visa_paths = sorted(glob.glob(visa_dir + "/*.JPG"))
    ...
    def __len__(self):
        return len(self.visa_paths)

    def __getitem__(self, index):
        image = Image.open(self.visa_paths[index]).convert("RGB")
        ...
        if self.is_train:
            ...
            image = self.final_preprocessing(image)
            dir_path, file_name = os.path.split(self.visa_paths[index])

...
class LOCODataset(Dataset):
    def __init__(
        ...
        if is_train:
            self.loco_paths = sorted(glob.glob(loco_dir+"/*.png"))
    ...
    def __len__(self):
        return len(self.loco_paths)

    def __getitem__(self, index):
        image = Image.open(self.loco_paths[index]).convert("RGB")
        ...
        if self.is_train:
            ...
            image = self.final_preprocessing(image)
            dir_path, file_name = os.path.split(self.loco_paths[index])
            ...
            if base_dir == "good":
                ...
                mask_file_name = os.path.join(file_name.split(".")[0], "000.png")

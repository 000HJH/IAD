...
class BTadDataset(Dataset):
    def __init__(
        ...
        if is_train:
            self.btad_paths = sorted(glob.glob(btad_dir+"/*.bmp") + glob.glob(btad_dir + "/*.png"))
        ...
        self.final_tensor_preprocessing = transforms.ToTensor()
        
    def __len__(self):
        return len(self.btad_paths)

    def __getitem__(self, index):
        image = Image.open(self.btad_paths[index]).convert("RGB")
        ...
        if self.is_train:
            ...
            # dtd_image = self.final_preprocessing(dtd_image) #$
            
            image = self.final_tensor_preprocessing(image) #값 정규화는 X
            dtd_image = self.final_tensor_preprocessing(dtd_image)
            return {"img_aug": aug_image, "img_origin": image, "mask": aug_mask, "dtd": dtd_image} #dtd
        else:
            image = self.final_preprocessing(image)
            dir_path, file_name = os.path.split(self.btad_paths[index])
            ...
            if base_dir == "ok":
                ...
                if not os.path.exists(mask_path): 
                    mask_path = os.path.join(dir_path, "../../ground_truth/", base_dir)

...
def main():
    ...
    args.config=args.config.format(args.dataset) #빈 {}에 formatting
    ...
    model = ModelHelper(config.net) #'backbone': WideResNet50
    model.cuda()
    ...
    for epoch in range(last_epoch, config.trainer.max_epoch):
        ...
        ### 500 epoch 저장
            ...
            torch.save(optimizer.state_dict(), os.path.join(config.checkpoints_path, args.class_name, f"optim_{epoch+1}.pth.tar"))

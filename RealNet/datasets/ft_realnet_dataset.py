...
def build_realnet_dataloader(cfg, training,distributed=True):
    ...
    if training:
        transform_fn = TrainBaseTransform(
            ...
            cfg["input_size"],False,False,False
        )
    else:
...
class RealNetDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # read image
        filename = meta["filename"] #bottle/train/good/141.png
        label = meta["label"]
        ...
        if meta.get("clsname", None):
            ...
            input["clsname"] = filename.split("/")[-4] #bottle

        image = Image.fromarray(image, "RGB")

        # read / generate mask

        if meta.get("maskname", None): #"maskname"이라는 키가 존재하면
        ...
        if self.training:
            ...
            input.update({'gt_image':gt_image}) #Backbone에서 feature 추출

        image_anomaly_type =self.choice_anomaly_type()
        assert image_anomaly_type in ['normal','dtd','sdas']
        

        if image_anomaly_type!='normal': #dtd, sdas
            ...
            anomaly_source_img, target_foreground_mask, factor= self.generate_anomaly(np.array(image), self.dataset, input["clsname"],image_anomaly_type, filename)
            anomaly_source_img = Image.fromarray(anomaly_source_img, "RGB") #외부 이상치

            fg_mask = Image.fromarray(np.array(target_foreground_mask*255.0).astype(np.uint8), "L")
            # fg_mask = Image.fromarray(np.array(target_foreground_mask).astype(np.uint8), "L")

            fg_mask = self.transform_fn(fg_mask)
            
            fg_mask = transforms.ToTensor()(fg_mask)
            fg_mask = torch.round(fg_mask) #$

            anomaly_source_img = transforms.ToTensor()(anomaly_source_img) #anomaly_source_img는 transfom_fn 수행X

        
        else: # training 아닌 경우 (미사용)$
            anomaly_source_img = copy.deepcopy(transforms.ToTensor()(image))
            fg_mask = torch.ones_like(transforms.ToTensor()(mask))
            factor = np.array(1, dtype=np.float64)

        image = transforms.ToTensor()(image)#$
        factor = torch.tensor(factor)
                
        # if self.normalize_fn: #$
        #     anomaly_source_img = self.normalize_fn(anomaly_source_img)

        # input.update({"image": image, "mask": mask, "anomaly_type":image_anomaly_type})

        input.update({"image": image, "anomaly_source_img": anomaly_source_img, "fg_mask": fg_mask,"factor": factor, "anomaly_type":image_anomaly_type, "mask":transforms.ToTensor()(mask)}) #mask는 test에서만 사용 (test는 원본파일 이용)
    
        return input


    def generate_anomaly(self, img, dataset,subclass, image_anomaly_type ,filename, get_mask_only=False):
        '''
        ...
        target_foreground_mask = self.generate_target_foreground_mask(img,dataset, subclass, filename)
        """#$"""
        ...
        anomaly_source_img, factor = self.anomaly_source(img=img,
                                                 anomaly_type=image_anomaly_type)
        anomaly_source_img = anomaly_source_img.astype(np.uint8)

        # return anomaly_source_img, mask        
        return anomaly_source_img, target_foreground_mask, factor# anoma..img는 원본 소스 이미지로 변경


    def generate_target_foreground_mask(self, img: np.ndarray, dataset:str,subclass: str, filename: str) -> np.ndarray:

        
        #/data/jiho/IAD/datasets/visa/fg_mask/candle/0000.JPG
        ...
        if dataset=='mvtec':
            ...
            #####
                ...
                target_foreground_mask_path = os.path.join('../datasets/',dataset,'fg_mask',filename.split('/')[-4],filename.split('/')[-1])
                target_foreground_mask = cv2.imread(target_foreground_mask_path, cv2.IMREAD_GRAYSCALE)
                # print(os.path.isfile(target_foreground_mask_path))
                return target_foreground_mask/255
            #####    
            """
            if subclass=='pill':
    ...
    def anomaly_source(self, img: np.ndarray,
        ...
        return anomaly_source_img, factor

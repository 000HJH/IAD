...
class BTadDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_fg = PIL.Image.open("../datasets/mvtec/fg_mask/carpet/000.png") #전체 1인 이미지
        aug = torch.tensor([1]) #test시 (어차피 안 씀) 
        ###
        if self.split == DatasetSplit.TRAIN:
            ...
            try:
                '''항상 fg mask 반환'''
                mask_fg = PIL.Image.open(fgmask_path)
            except: #fg file 없는 경우(btad, loco)
                pass
    ...
    def get_image_data(self):
        ...
        for anomaly in anomaly_types:
            ...
            if self.split == DatasetSplit.TEST and anomaly != "ok":
                ...
                maskpaths_per_class[self.classname]["ok"] = None

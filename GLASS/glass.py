...
class GLASS(torch.nn.Module):
        ...
        for i_epoch in pbar:
            ...
            if (i_epoch + 1) % self.eval_epochs == 0:
                ...
                if best_record is None:
                    ...
                    ckpt_path_best = os.path.join(self.ckpt_dir, "ckpt_best_{}.pth".format(i_epoch))
                    torch.save(state_dict, ckpt_path_best) #best 모델 갱신
                    shutil.rmtree(eval_path, ignore_errors=True)
                    shutil.copytree(train_path, eval_path)
            ...
            pbar.set_description_str(pbar_str) #epoch당 1회
            # torch.save(state_dict, ckpt_path_save)
        torch.save(state_dict, ckpt_path_save) #$ 마지막 모델 저장
        torch.save(self.proj_opt.state_dict(), os.path.join(self.ckpt_dir, "proj_optim.pth"))
        torch.save(self.dsc_opt.state_dict(), os.path.join(self.ckpt_dir, "dsc_optim.pth"))
    ...
    def _train_discriminator(self, input_data, cur_epoch, pbar, pbar_str1):
        ...
        for i_iter, data_item in enumerate(input_data):
            ...
            true_points = torch.cat([fake_feats[mask_s_gt[:, 0] == 0], true_feats], dim=0) #$ concat -> cat
            c_t_points = torch.cat([center[mask_s_gt[:, 0] == 0], center], dim=0)
            dist_t = torch.norm(true_points - c_t_points, dim=1)
            r_t = torch.tensor([torch.quantile(dist_t, q=self.radius)]).to(self.device)
    
            ## 피처에 grad ascent 가우시안 노이즈 step
            for step in range(self.step + 1):
                ...
                if (step + 1) % 5 == 0:
                    ...
                    # r = r_t if self.svd == 1 else 0.5 #r_t가 tensor라 clamp시 오류
                    r = r_t.item() if self.svd == 1 else 0.5 #$
            ...
            r = r_t.item() if self.svd == 1 else 1 #$
            ...
            pix_true = torch.cat([fake_scores.detach() * (1 - mask_s_gt), true_scores.detach()])
            pix_fake = torch.cat([fake_scores.detach() * mask_s_gt, gaus_scores.detach()])
    ...
    def tester(self, test_data, name):
        # ckpt_path = glob.glob(self.ckpt_dir + '/ckpt_best*') #원본 best 성능
        # ckpt_path = glob.glob(self.ckpt_dir + '/ckpt.pth') 
        ckpt_path = os.path.join(self.ckpt_dir, 'ckpt.pth') ##glob으로 -> best가 추출되었음(04.03 변경)
        ...
        if len(ckpt_path) != 0:
            ...
            # epoch = int(ckpt_path[0].split('_')[-1].split('.')[0]) #원본 best 성능
            epoch = 640 #$
        else:
        ...
        if len(masks_gt) > 0:
            ...
            if path == 'eval':
                try:
                    ...
                    pixel_pro = metrics.compute_pro(segmentations, np.squeeze(np.array(masks_gt)))
                except:
                    pixel_pro = 0.
            else:
                pixel_pro = 0.
